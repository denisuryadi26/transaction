            ...
            <div class="card" style="min-height:85vh">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div style="width: 25.00%; height: 25.00%; border:1px solid rgb(243, 243, 243)" class="mb-4">
                            <div class="productCard">
                                <div class="view overlay">
                                    <form action="{{url('/transaction/addproduct', $product->id)}}" method="POST">
                                        @csrf
                                        @if($product->qty == 0)
                                        <img class="card-img-top gambar" src="{{ asset('/img/gambar').'/'.$product->image }}"
                                            alt="Card image cap">
                                        <button class="btn btn-block btn-primary btn-sm cart-btn disabled"><i
                                                class="fas fa-cart-plus"></i> Add To Chart</button>
                                        @else
                                        <img class="card-img-top gambar" src="{{ asset('/img/gambar').'/'.$product->image }}"
                                            alt="Card image cap" style="cursor: pointer"
                                            onclick="this.closest('form').submit();return false;">
                                        <button type="submit" class="btn btn-block btn-primary btn-sm cart-btn"><i
                                                class="fas fa-cart-plus"></i> Add To Chart</button>
                                        @endif
                                    </form>
                                </div>
                                <div class="card-body">
                                    <label class="card-text text-center font-weight-bold"
                                        ...
                                        {{ Str::words($product->name,4) }} <br>
                                        Stok : {{$product->qty}} </label>
                                    <p class="card-text text-center">Rp. {{ number_format($product->sell_price,2,',','.') }}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <form action="{{ url('/transaction/clear') }}" method="POST">
                                @csrf
                                <button class="btn btn-info btn-lg btn-block" style="font-size:15px"
                        ...
                        <div class="col-sm-4">
                            <a class="btn btn-primary btn-lg btn-block"
                                style="font-size:15px" href="{{url('/transaction/history')}}" target="_blank">History</a>
                            <!-- Kembangkan sendiri ya bagian ini, logikanya kita simpan cartnya sementara dalam databse ntar kalau butuh keluarin lagi-->
                        </div>
                        <div class="col-sm-4">
                            <button class="btn btn-success btn-lg btn-block" style="font-size:15px"
